...
export default function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDeleteProperty = async () => {
    try {
      ...
      window.location.href = "/pms/properties";
    } catch (err: any) {
      toastError(handleApiError(err));
    } finally {
  ...
  if (propertyError) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center gap-4 text-muted-foreground">
        ...
        <Link href="/pms/properties" className="text-sm text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300">Go Back</Link>
      </div>
    );
  }
      ...
      <div className="min-h-[400px]">
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <Link
              href={`/pms/properties/${propertyId}/connect`}
                ...
                <div key={conn.id} className="bg-card border border-border rounded-2xl p-6 relative overflow-hidden group shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="mt-6 pt-4 border-t border-border space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleOperation(conn.id, "activate-rooms")}
                        disabled={!!opLoading}
                        className="text-[10px] font-bold px-3 py-1.5 bg-blue-500/10 hover:bg-blue-500/20 text-blue-600 dark:text-blue-400 rounded border border-blue-500/20 uppercase tracking-tight flex items-center gap-1.5 transition-all"
                      >
                        {opLoading === `${conn.id}-activate-rooms`
                          ? <Loader2 className="w-2.5 h-2.5 animate-spin" />
                          : <Hotel className="w-2.5 h-2.5" />}
                        Activate Rooms
                      </button>
                      <button
                        onClick={() => handleOperation(conn.id, "rooms-cancellation")}
                        disabled={!!opLoading}
                        className="text-[10px] font-bold px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 rounded border border-amber-500/20 uppercase tracking-tight flex items-center gap-1.5 transition-all"
                      >
                        {opLoading === `${conn.id}-rooms-cancellation`
                          ? <Loader2 className="w-2.5 h-2.5 animate-spin" />
                  ...
                  {(conn.ota_name?.toLowerCase() === "booking" || conn.ota_name?.toLowerCase() === "expedia") && (
                    <div className="mt-5 flex flex-wrap gap-2">
                      {conn.ota_name?.toLowerCase() === "booking" && (
                        <>
                          <Link
                            href={`/pms/properties/${propertyId}/connections/${conn.id}/booking`}
                          ...
                          <Link
                            href={`/pms/properties/${propertyId}/connections/${conn.id}/reporting`}
                      ...
                      {conn.ota_name?.toLowerCase() === "expedia" && (
                        <Link
                          href={`/pms/properties/${propertyId}/connections/${conn.id}/expedia`}
